...
def adaClassify(dataToClass, classifier):
    ...
    for i in range(len(classifier)):   # 遍历所有分类器，进行分类
        ...
        print("aggClassEst in adaClassify function:\n", aggClassEst)
...
if __name__ == "__main__":
    ...
    testMat, testLabelMat = loadDataSet(
        "E:\\Code\\local_code\\机器学习实战\\DataSet\\horseColicTest2.txt")
    weakClassArr, aggClassEst = adbBoostTrainDS(dataMat, labelMat)
    print("weakClassArr:", weakClassArr)
    predictions = adaClassify(dataMat, weakClassArr)
    errArr = np.mat(np.ones((len(dataMat), 1)))
    print("训练集的错误率: %.3f%%" % float(
        errArr[predictions != np.mat(labelMat).T].sum() / len(dataMat) * 100))
    predictions = adaClassify(testMat, weakClassArr)
    errArr = np.mat(np.ones((len(testMat), 1)))
    print("测试集的误差率: %.3f%%" % float(
        errArr[predictions != np.mat(testLabelMat).T].sum() / len(testMat) * 100))
