...
FLAGS = tf.flags.FLAGS  # 用于支持接受命令行传递参数，相当于接受argv
# FLAGS._parse_flags()
# print("\nParameters:")
...
def preprocess():
    ...
    vocab_processor = learn.preprocessing.VocabularyProcessor(
        max_document_length)  # 使用tf中的函数创建词汇表
...
def train(x_train, y_train, vocab_processor, x_dev, y_dev):
    ...
    with tf.Graph().as_default():
        session_conf = tf.ConfigProto(
          allow_soft_placement=FLAGS.allow_soft_placement,  # 允许动态分配GPU内存
          log_device_placement=FLAGS.log_device_placement)  # 打印出设备信息
...
# 训练过程的可视化：tensorboard --logdir E:\NLP\code\cnn-text-classification\runs\1547982253\summaries\
